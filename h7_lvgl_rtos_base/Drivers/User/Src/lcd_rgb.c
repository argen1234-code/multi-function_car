...
*					2.	������Ҫ��ʾ����ɫ������ʹ�� LCD_SetColor(0xff0000FF) ����Ϊ��ɫ
*					3. �����ö�Ӧ�ı���ɫ������ʹ�� LCD_SetBackColor(0xff000000) ����Ϊ��ɫ�ı���ɫ
*					4. ʹ��ʾ�� LCD_DisplayString( 10, 10, "1111KE") ������ʼ����Ϊ(10,10)�ĵط���ʾ�ַ���"1111KE"
*
***************************************************************************************************************/
...
*					2.	������Ҫ��ʾ����ɫ������ʹ�� LCD_SetColor(0xff0000FF) ����Ϊ��ɫ
*					3. �����ö�Ӧ�ı���ɫ������ʹ�� LCD_SetBackColor(0xff000000) ����Ϊ��ɫ�ı���ɫ
*					4. ʹ��ʾ�� LCD_DisplayChinese( 10, 10, "1111�Ƽ�STM32") ��������(10,10)��ʾ�ַ���"1111�Ƽ�STM32"
*
***************************************************************************************************************/
 ...
 * @retval      ��
 */
void lcd_color_fill_fast(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t *color)
{
    uint32_t pixel_index = 0;
    
    // �߽���
    if (sx >= LCD_Width || sy >= LCD_Height || ex >= LCD_Width || ey >= LCD_Height) {
        return;
    }
    ...
    if (LCD.ColorMode == LTDC_PIXEL_FORMAT_RGB565) {
        ...
        for (uint16_t y = sy; y <= ey; y++) {
            for (uint16_t x = sx; x <= ex; x++) {
                ...
                if (LCD.Direction == Direction_H) {
                    // ����ģʽ
                    pixel_addr = LCD.LayerMemoryAdd + 2 * (y * LCD_Width + x);
                } else {
                    // ����ģʽ
                    pixel_addr = LCD.LayerMemoryAdd + 2 * ((LCD_Height - x - 1) * LCD_Width + y);
                }
                
                // ֱ��д���Դ�
                *(__IO uint16_t*)pixel_addr = color[pixel_index];
                pixel_index++;
            }
        }

    } else {
        // ������ʽʹ��ͨ�õ�LCD_DrawPoint
        for (uint16_t y = sy; y <= ey; y++) {
            for (uint16_t x = sx; x <= ex; x++) {
                LCD_DrawPoint(x, y, color[pixel_index]);
                pixel_index++;
            }
        }
    }
...
/**
  ...
  * @retval ��
  */
void DMA2D_DrawPixel(uint16_t x, uint16_t y, uint32_t color)
{
    /* �ȴ�֮ǰ��DMA2D������� */
    ...
    if(LCD.Direction == Direction_H)  // ����
    {
        address = LCD.LayerMemoryAdd + LCD.BytesPerPixel * (LCD_Width * y + x);
        DMA2D->OOR = 0;  // ���㣬��ƫ��Ϊ0
    }
    else  // ����
    {
        address = LCD.LayerMemoryAdd + LCD.BytesPerPixel * ((LCD_Height - x - 1) * LCD_Width + y);
        DMA2D->OOR = 0;  // ���㣬��ƫ��Ϊ0
    }
    
    DMA2D->OMAR = address;  // Ŀ��洢����ַ
    
    /* ���ô���ߴ磺1������ */
    DMA2D->NLR = (1 << 16) | 1;  // ����=1���߶�=1
...
/**
 * @brief       �޸���DMA2D��ɫ��亯�� - ��������֤�ĵ�ַ����
 * @param       sx,sy: ��ʼ����
 * @param       ex,ey: ��������  
 * @param       color: Ҫ������ɫ�����׵�ַ
 * @retval      ��
 */
void lcd_color_fill_dma2d(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t *color)
{
	    uint32_t pixel_index = 0;
    ...
    for (uint16_t y = sy; y <= ey; y++) {
        for (uint16_t x = sx; x <= ex; x++) {
            DMA2D_DrawPixel(x, y, color[pixel_index]);
            pixel_index++;
        }
    }
...
/**
 * @brief       ʹ��DMA2D�����Σ�����ɫ����/ͼ�����ݣ������ã�ͼ����˺��
 * @param       sx, sy: ��ʼ����
 * @param       ex, ey: ��������  
 * @param       colors: ��ɫ�����׵�ַ
 * @retval      ��
 */
//void lcd_color_fill_dma2d(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t *colors)
//{
...
//    /* ����DMA2D - �洢�����洢��ģʽ */
//    DMA2D->CR = DMA2D_M2M;
//    DMA2D->FGPFCCR = LCD.ColorMode;  // ǰ������ɫ��ʽ
//    DMA2D->FGMAR = (uint32_t)colors; // Դ���ݵ�ַ����ɫ���飩
//    DMA2D->FGOR = 0;                 // ǰ������ƫ�ƣ��������ݣ�
//    
//    DMA2D->OPFCCR = LCD.ColorMode;   // �����ɫ��ʽ
//    
//    /* ����Ŀ���ַ */
//    uint32_t address;
//    if(LCD.Direction == Direction_H) {  // ����
//        address = LCD.LayerMemoryAdd + LCD.BytesPerPixel * (LCD_Width * sy + sx);
//        DMA2D->OOR = LCD_Width - width;  // ��ƫ��
//    } else {  // ����
//        address = LCD.LayerMemoryAdd + LCD.BytesPerPixel * ((LCD_Height - ex - 1) * LCD_Width + sy);
//        DMA2D->OOR = LCD_Width - height;  // ����ģʽ����ƫ��
//    }
//    
//    DMA2D->OMAR = address;  // Ŀ��洢����ַ
//    
